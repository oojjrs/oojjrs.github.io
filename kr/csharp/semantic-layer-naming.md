...
description: "게임 프로젝트와 서버 애플리케이션의 비즈니스 로직에서 식별자 경계를 소유·구조·관계·역할 계층으로 읽는 명명 규칙입니다."
...
이 판단법은 특정 언어·엔진·서버 프레임워크의 표면 문법에 의존하지 않는다. 다만 적용 대상은 게임 프로젝트 자체나 서버 애플리케이션 자체의 비즈니스 로직이며, 엔진·라이브러리·저수준 계층까지 일반화하지 않는다.
...
이 규칙은 게임 프로젝트 자체 또는 서버 애플리케이션 자체의 비즈니스·제품 로직에서 생성하거나 변경하는 first-party 이름에 적용한다. 해당 계층의 타입, 멤버, module, package, file, resource, 직렬화 객체, DTO, persistence 타입, service 역할처럼 코드 식별자와 코드가 소유하는 이름을 포함한다.

서비스 중 도메인 규칙과 콘텐츠가 계속 늘어나는 코드에는 좁은 책임마다 유니크한 전담 타입과 파일이 생긴다. 각 단위는 책임이 충분히 좁아지면 기능이 계속 붙기보다 안정되고, 새로운 요구는 sibling 전담 단위를 추가하는 형태로 쌓이는 경향이 있다. 의미 계층을 구조 좌표로 드러내는 방식은 이렇게 이름 패밀리의 양이 계속 늘어나는 환경을 탐색하기 위한 규칙이다.

엔진, 런타임, 프레임워크, 재사용 라이브러리, 인프라 기반 요소, 완전 저수준 코드에는 이 규칙을 적용하지 않는다. 그 계층에서는 재사용 API의 모양, 추상화 경계, protocol이나 ABI, 할당·성능 제약, 플랫폼의 고유 어휘처럼 그 계층만의 사정이 우선하므로 각자의 관례를 따른다.
