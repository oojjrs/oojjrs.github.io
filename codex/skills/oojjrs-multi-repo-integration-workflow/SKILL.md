...
1. Use `$oojjrs-project-finish-work` separately for each explicitly authorized scoped commit, including its canonical commit-message check in every repository. A local commit in one repository does not authorize a commit, push, deployment, or release in another.
