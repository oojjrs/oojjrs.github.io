...
5. Edit ordinary C# source with the repository's text workflow. For tasks that only create or edit `.cs` source, do not use `$unity-cli` or the Editor for script import, `.meta` generation, compilation, or validation; leave new script metadata to the next normal project import. When the requested scope separately requires Unity-side asset moves, renames, imports, or serialized changes, use `$unity-cli` and Unity asset APIs to preserve existing GUIDs. Never fabricate or hand-edit a script `.meta`. Follow an evidenced package policy that intentionally omits script metadata, and include every Unity-generated in-scope companion with its script.
