...
3. Treat every pre-existing or concurrently appearing working-tree or index change and untracked item as protected from Codex content edits unless the common rules identify it as task-owned temporary or generated output. Protection alone does not decide whether unchanged bytes are eligible for staging; the finish workflow reconciles that separately. Inspection, validation, a `TODO`, a defect, a missing link, staging, or commit never grants authority to edit outside the current task's content scope.
