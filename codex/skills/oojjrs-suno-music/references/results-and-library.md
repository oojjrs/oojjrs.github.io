...
If the page is filtered, paginated, or sorted away from new items, use the current **Search clips** control or one focused filter change to expose the newest relevant results, then verify by ID. Do not clear unrelated user filters without a task-specific reason. Distinguish an **Upload** source from **Full Song** and **Extend** results by both stable ID and visible badge; identical titles do not establish identity.
...
When attached Audio initially opens as **Cover**, change the derivative type through the visible `Cover에서 조건 유형 변경` menu and verify **연장** before submitting an Extend. Preserve the prefilled **Extend from** point unless the user requested a different one; an observed value such as `24.0` belongs to that source.
...
- When another Extend or generated stage will use a new variant as its source, stop after each batch for audition and explicit source selection. For one already authorized Extend whose requested outcome includes Get Whole Song, that assembly does not require another approval; verify the exact completed result ID before assembling it. A technically complete but rejected stage must not become the source of a later generation chain.
