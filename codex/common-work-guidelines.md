...
8. If the git repository is linked 1:1 to a GitHub Project, use that Project as the task board: find/create the task card, convert a draft card to an issue when moving it to In Progress, update status/notes while working, use real newlines in card/issue bodies instead of literal `\n`, and report if no board/card update was possible.
