...
24. In a `MonoBehaviour`, treat Unity message functions as an implicit first inherited group and implement them before explicitly inherited parent and interface members. Sort Unity messages alphabetically by function name, then use parent > child order, parents in inheritance declaration order, multiple interfaces and implementations alphabetically, and finally local members. In types that do not use Unity messages, constructors still come first among functions.
