...
5. 모든 로그의 최종 출력에는 행위 주체자가 가장 앞에 `{actor}> ` 형태로 있어야 한다. 호출 문자열에 직접 작성하거나 태그가 설정된 로거가 시스템적으로 붙일 수 있으며, 로거가 붙이는 경우 호출부에서 중복 작성하지 않는다. 주체자는 객체 이름, 타입 이름, 사용자·세션 ID, 연결 이름, 컨트롤러 이름처럼 사건을 발생시킨 시스템이나 인스턴스를 식별해야 한다.
...
9. 예외 객체는 가능한 경우 문자열로 풀어 쓰지 말고 행위 주체자와 예외 객체를 함께 받을 수 있는 공통 예외 API에 전달한다. 사용 중인 API가 주체자를 받을 수 없다면 태그가 붙는 래퍼를 만든다.
...
- 주체가 프로세스 전체에서 하나뿐이고 문맥상 절대 모호하지 않은 전역 부팅 로그는 주체를 생략할 수 있다. 다만 비교·집계·다중 인스턴스 가능성이 있으면 애플리케이션이나 시스템 태그를 붙인다.
...
```csharp
Debug.Log($"{applicationName}> APPLICATION START : {Application.version} ({Application.platform})");
...
- 주체자 없는 전역·네트워크 축약 로그가 정말 모호하지 않은가?
- 민감한 값이 포함되지 않았는가?
